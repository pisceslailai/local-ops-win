...
def _win_process_table(refresh=False):
    ...
    script = (
        ...
        "$cpu[[int]$_.IDProcess]=[double]$_.PercentProcessorTime }; "
        "Get-CimInstance Win32_Process | Select-Object ProcessId,"
        "ParentProcessId,Name,ExecutablePath,CommandLine,CreationDate,"
        "WorkingSetSize,@{Name='CpuPercent';Expression={ "
        "if($cpu.ContainsKey([int]$_.ProcessId)){$cpu[[int]$_.ProcessId]}"
...
def _win_stop_process(pid, tree=True, force=False):
    """Windows 停止策略：先普通 taskkill；无窗口控制台进程再降级 /F。"""
    if force:
...
def _scan_listeners_windows():
    """快速解析 netstat；状态名不参与判断，因此不受系统语言影响。

    极少数裁剪系统若 netstat 不可用，再回退语言无关的
    Get-NetTCPConnection。
    """
    found = _parse_netstat_output(
        run_cmd(["netstat", "-ano", "-p", "tcp"]))
    if found:
        return found
    script = (
    ...
    text = _win_powershell(script, timeout=SUBPROCESS_TIMEOUT * 2)
    try:
    ...
    if isinstance(items, list):
        ...
        for item in items:
            ...
            if pid <= 0 or not (1 <= port <= 65535):
                continue
            host = str(item.get("LocalAddress") or "").strip("[]")
...
def stop_pid_tree(pid, sig=signal.SIGTERM, force=False):
    ...
    Windows：无 POSIX 进程组，使用 taskkill /T。无窗口控制台进程通常不
    接受普通 taskkill，因此失败后会明确降级为 /F；界面与文档提示风险。
    """
    if IS_WIN:
...
def _pick_path_windows(what):
    ...
    script += (
        ...
        "{ $f.$property } else { '__CANCELED__' } "
        "} finally { $f.Dispose(); $owner.Close(); $owner.Dispose() }")
...
def resolve_app_stop_target(app, listeners=None):
    ...
    if legacy_pid:
        if app.get("attached") and IS_WIN:
            table = _win_process_table()
            members = (_win_tree_of(legacy_pid, table)
